...
add_another_row <- function(session_dir, subj_name, subj_interface_file_name, condition, temp_df) { 
  ...
  col_names <- c("PP", "HR", "BR", "D.EDA", "D.HR") 
  ...
  for (col in col_names) { 
    ...
    if (col %in% names(temp_df)) { 
      ...
      if (length(a) != 0) { 
        ...
        #   vec_elem <- paste(vec_elem, " O")
        # } else {
        #   vec_elem <- paste(vec_elem, " X")
        # }
...
format_final_df <- function() { 
  ...
  col_names_final <- c("University", "Subject", "Group", "PP", "HR", "BR", "D.EDA", "D.HR") 
  ...
  totals <- result_df %>% 
    summarize(sumPP = sum(grepl(" O", PP), na.rm = TRUE), 
              sumHR = sum(grepl(" O", HR), na.rm = TRUE), 
              sumBR = sum(grepl(" O", BR), na.rm = TRUE), 
              sumD.EDA = sum(grepl(" O", D.EDA), na.rm = TRUE), 
              sumD.HR = sum(grepl(" O", D.HR), na.rm = TRUE)) 
  ...
  colnames(TOTALS) <- c("PP", "HR", "BR", "D.EDA", "D.HR", "University", "Subject", "Group") 
  TOTALS <- TOTALS[c("University", "Subject", "Group", "PP", "HR", "BR", "D.EDA", "D.HR")] 
...
Evaluating Criteria:  
O: Groups `IF` and `BF` have `Resting Baseline` (R) as the lowest mean value.  
O: Groups `BN` and `BN` have `Resting Baseline` (R) or `Break`(B) as the lowest mean value.  
X: Above conditions NOT met. 
