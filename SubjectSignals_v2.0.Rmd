---
title: "Sensor Channel per Subject - All Sessions" 
output: pdf_document 
...
do_the_actual_plotting_now <- function(df, subj_name, condition, col_name, n) { 
  ...
  if (!all(is.na(df[[col_name]]))) { 
    ...
    if (n == 3) { 
      temp_df <- df %>% 
        group_by(Session) %>% 
        filter(Session %in% c('RestingBaseline', 'BaselineWriting', 'DualTask')) %>% 
    ...
    if ("BaselineWriting" %in% temp_df$Session) { 
      colors <- c(colors, "#0082C8") 
    } 
    if ("StressCondition" %in% temp_df$Session) { 
...
figure_out_title <- function(subj_name, condition, col_name) { 
  ...
  if (isMatch(condition, 'IH')) { 
    part_one <- 'Intermittent High' 
  } else if (isMatch(condition, 'IL')) { 
    part_one <- 'Intermittent Low' 
