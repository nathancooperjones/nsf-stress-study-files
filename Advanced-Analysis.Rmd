...
testing_df <- read_csv("~/Desktop/nsf-stress-study-files/testing_df.csv", col_types = cols()) 
...
full_no_changing_df <- read_csv("~/Desktop/nsf-stress-study-files/full_df.csv", col_types = cols()) 
...
print(anova_df %>% 
  ungroup(Group) %>% 
  mutate(Group = factor(Group, levels = c("IH", "IL", "BH", "BL"))) %>% 
  group_by(Group) %>% 
...
print(anova_df %>% 
  ...
  ggplot(aes(x = Measure, y = Difference, fill = Group)) + 
    ...
    scale_fill_manual(labels = c("IF", "IR", "BF", "BR"), values = wes_palette("Darjeeling1")) 
) 
...
Our Linear Model: 
$$ 
 Writing Quality = 1 + Group + Session + 1|Subject 
$$ 
...
essay_score_df <- read_csv("~/Desktop/nsf-stress-study-files/essay_score_master_file.csv", col_types = cols()) 
...
# CREATING LINEAR MODELS 
fit_writing_quality <- lme(Score ~ 1 + Group + Session, data = full_df, random = ~1|Subject, na.action = na.omit) 
...
levels(full_df$Group) <- c("IF", "IR", "BF", "BR") 

g1 <- full_df %>% 
  ggplot(aes(x = Group, y = Score, fill = Session)) + 
    geom_boxplot() + 
    theme_bw() + 
    ggtitle(" ") + 
    ylab("ETS Score") + 
...
full_df <- full_df %>% 
  mutate(Group = factor(Group, levels = c("BR", "IR", "BF", "IF"))) 
