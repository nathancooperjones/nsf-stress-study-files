...
knitr::kable(tbl_df(result_df %>% mutate(Group = factor(Group, levels = c("IH", "BH", "IL", "BL", "...."))) %>% arrange(Group))) %>% 
...
Evaluating Criteria:  
✅: Groups `IH` and `BH` have `Resting Baseline` (⚫) as the lowest mean value.  
✅: Groups `IL` and `BL` have `Resting Baseline` (⚫) or `Stress Condition` (🔶) as the lowest mean value.  
