  ...
  for (comp in comparison_levels) { 
    ...
    g1 <- ggplotGrob(labelling_df %>% 
            ...
            ggplot(aes_string(x = "Measurement", y = comp)) + # have to use aes_string here when using a string for an aesthetic 
              ...
              scale_x_discrete(labels=c("ln(PP)")) + # make sure we have the right label for this specific graph 
    ...
    g2 <- ggplotGrob(labelling_df %>% 
            ...
            ggplot(aes_string(x = "Measurement", y = comp)) +
              ...
              scale_x_discrete(labels=c("ln(D.EDA)", "ln(N.EDA)")) + 
  ...
  for (comp in comparison_levels) { 
    ...
    g1 <- ggplotGrob(labelling_df %>% 
            ...
            ggplot(aes_string(x = "Measurement", y = comp)) + 
              ...
              theme(axis.title.x=element_blank(), axis.text.x = element_text(size=10)) + 
              stat_summary(fun.data = give.n, geom = "text") + 
              scale_x_discrete(labels=c("ln(PP)")) + 
              scale_y_continuous(expand = c(0.15, 0, 0.3, 0)) + 
...
anova_df <- tibble() 
anova_df <- session_compare_df[c("Subject", "Condition", "Measurement", diff1, diff2)] %>% 
  filter(Measurement == measure) %>% 
  na.omit() 

anova_df <- anova_df %>% 
  gather(Measure, Difference, -Subject, -Condition, -Measurement) %>% 
  mutate(Measure = factor(Measure, levels = c(diff1, diff2))) 
...
print(anova_df %>% 
  ...
  mutate(Condition = factor(Condition, levels = condition_vec)) %>% 
  group_by(Condition) %>% 
  ggplot(aes(x = Measure, y = Difference, fill = Condition)) + 
    ...
    scale_y_continuous(expand = c(0.1, 0, 0.1, 0)) + 
    scale_x_discrete(labels = c("SingleTask", "DualTask")) 
) 

print(anova_df %>% arrange(Difference)) 
...
print(anova_df %>% 
  ...
  group_by(Condition) %>% 
  ggplot(aes(x = Condition, y = Difference, fill = Measure), color = "black") + 
    geom_boxplot(color = "#000000") + 
    theme_bw() + 
    geom_hline(yintercept=0, linetype="dashed", color = "black", alpha = 0.6, size=1) + 
    theme(axis.title.x=element_blank(), 
...
```{r echo = FALSE, warning = FALSE, message = FALSE} 
write.table(final_testing_df, file = "testing_df.csv", row.names=F, sep = ',') 
``` 
