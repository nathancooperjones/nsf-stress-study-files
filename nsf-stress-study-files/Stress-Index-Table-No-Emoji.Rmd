...
#full_df <- read_csv("T:/Google Drive/University of Houston/CS - UH/@Research - CPL/@Projects/NsfStressDataCuration/nsf-stress-study-scripts/@Datasets/full_df.csv", col_types = cols()) 
result_df <- tibble() 
...
format_final_df <- function() { 
  ...
  totals <- result_df %>% 
    summarize(sumPP = sum(grepl(" O", PP), na.rm = TRUE), 
              ...
              sumN.EDA = sum(grepl(" O", N.EDA), na.rm = TRUE), 
              sumN.HR = sum(grepl(" O", N.HR), na.rm = TRUE))  
